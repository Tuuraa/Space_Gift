...
async def worker(bot: Bot, loop):
    while True:
        try:
            ...
            users = clear_repeat(dbPay.get_users())

            for user in users:
                user_data = dbPay.get_data(user[0])
                ...
                for pay in user_data:
                    ...
                    if await Payment.get_order(pay[0]):
                        ...
                        if status_payment == "CANCELED":
                                ...
                                dbPay.change_status_for_cancel("CANCELED", pay[5], "CREDIT")
                            ...
                            # print(dbPay.get_status_credit(pay[5]))
                                ...
                                # Пополнение счетов
                                dbUser.add_depozit(user[0], pay[1])
                                dbUser.add_money(user[0], pay[1])
                                dbUser.add_gift_money(user[0], pay[1])

                                # Оповещение реферала
                                referrer_id = dbUser.get_referrer_of_user(user[0])
                                if referrer_id != "None":
                                    dep = pay[1] * .1
                                    ...
                                    await bot.send_message(
                                        referrer_id,
                                        f"Ваш реферал {dbUser.get_name(user[0])} пополнил баланс и вам подарили {dep} RUB."
                                    )
                                await bot.delete_message(user[0], pay[5])
                                ...
                                dbPay.change_status_for_cancel("OPERATION_COMPLETED", pay[5], "CREDIT")
                                #dbPay.cancel_request(pay[5], "CREDIT")

                                depozit = dbUser.get_deposit(user[0])

                                if depozit >= 5000:
                                    await clones.create_clones(pay[1])

                                # В случай первого пополнения
                                now_dep = dbUser.get_now_depozit(user[0])
                                if now_dep <= 0:
                                    dbUser.set_now_depozit(user[0], pay[1])
                                    response = "Супер 🙌 \n" \
